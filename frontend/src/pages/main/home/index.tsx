...
export const HomePage = () => {
  const [yearPage, setYearPage] = useState("2024");
  const page2024 = <Container>
    <Container fluid>
      ...
      <Row className="map justify-content-between">
        ...
        <Col className="map__steps" md={12} lg={3}>
          ...
          <p>
            Посетите мероприятия и получите новые знания о космосе
          </p>
        </Col>
      </Row>
      <Row className="archive justify-content-center">
        <h2>Дни Космоса 2023</h2>
        <Carousel year="2023" />
      </Row>
    ...
    <Container fluid>
      ...
      <Row className="present justify-content-center">
        ...
        <Col className="d-flex flex-column justify-content-center">
          ...
          <ul>
            <li>
              Получи на открытии <b>8 апреля</b> или распечатай флаер
            </li>
            <li>
  ...
  return <><div className={styles.yearRow}>{Object.keys(pages).reverse().map(year =>
    <Button variant="outline-primary" key={year}
      onClick={() => setYearPage(year)}
      className={classnames(styles.mainButton, styles.yearButton, "rounded-pill align-self-start", yearPage === year && styles.selectedYear)}>{year}</Button>
  )}</div>
    {pages[yearPage]}</>;
};
